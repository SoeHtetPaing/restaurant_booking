<?php
    function createTable ($database) {
        ...
        $sql = "create table if not exists restaurant (id int primary key auto_increment, name varchar(100), email varchar(100), phone varchar(15), address varchar(100), location int not null, password text, logo varchar(100), foreign key (location) references restaurant_location (id));";
        if ($database->query($sql) === false) return false;
        ...
        $sql = "create table if not exists booking (bid int primary key auto_increment, rid int not null, cid int not null, makedate date, maketime varchar(50), name varchar(100), phone varchar(15), bdate date, btime varchar(50), bill int, transaction varchar(100), status int, reject int, foreign key (rid) references restaurant (id), foreign key (cid) references customer (id));";
        if ($database->query($sql) === false) return false;

        $sql = "create table if not exists booking_chair (id int primary key auto_increment, bid int not null, cid int not null, cno varchar(100), foreign key (bid) references booking (bid), foreign key (cid) references restaurant_chair (cid));";
        if ($database->query($sql) === false) return false;

        $sql = "create table if not exists booking_menu (id int primary key auto_increment, bid int not null, mid int not null, qty int, foreign key (bid) references booking (bid), foreign key (mid) references restaurant_menu (mid));";
        if ($database->query($sql) === false) return false;
